-module(d_util_SUITE).

%% Note: This directive should only be used in test suites.
-compile(export_all).

-include("../src/d_util.hrl").
-include_lib("proper/include/proper.hrl").
-include_lib("common_test/include/ct.hrl").
-define(CHECKSPEC(M,F,N), true = proper:check_spec({M,F,N})).
-define(PROPTEST(A), true = proper:quickcheck(A())).

%%--------------------------------------------------------------------
%% COMMON TEST CALLBACK FUNCTIONS
%%--------------------------------------------------------------------

suite() ->
    [{ct_hooks,[cth_surefire]}, {timetrap,{minutes,1}}].

init_per_suite(Config) ->
    Config.

end_per_suite(_Config) ->
    ok.

init_per_group(_GroupName, Config) ->
    Config.

end_per_group(_GroupName, _Config) ->
    ok.

init_per_testcase(_TestCase, Config) ->
    Config.

end_per_testcase(_TestCase, _Config) ->
    ok.

groups() ->
    [{conversion, [parallel],
      [t_get_string,
       t_get_binary,
       t_get_integer,
       t_get_boolean,
       t_get_base36,
       t_get_base62]},


    {validation, [parallel],
     [t_required,
      t_validate_list_of_binaries,
      t_validate_email_address,
      t_validate_email_address_correct]},
     
    {list, [],
     [t_get_value_3,
      t_get_value_4,
      t_get_value_4_generated]},

    {uuid, [],
     [t_create_uuid]},

    {d_json, [parallel],
     [t_validate_boolean,
      t_validate_boolean_list,
      t_validate_boolean_list_generated,
      t_validate_float,
      t_validate_float_list,
      t_validate_integer,
      t_validate_integer_list,
      t_validate_integer_list_generated,
      t_validate_non_neg_integer,
      t_validate_non_neg_integer_list,
      t_validate_non_neg_integer_list_generated,
      t_validate_integer_range,
      t_validate_integer_range_generated,
      t_validate_binary,
      t_validate_binary_list,
      t_validate_binary_list_generated,
      t_validate_undefined,
      t_validate_undefined_generated,
      t_validate_null,
      t_validate_null_generated,
      t_validate_ignore,
      t_validate_atom,
      t_validate_atom_from_list,
      t_validate_url,
      t_validate_tcp_port,
      t_validate_utf8,
      t_validate_utf8_spec,
      t_validate_list_with,
      t_validate_area_code]}].

all() ->
    [{group, conversion},
     {group, validation},
     {group, list},
     {group, uuid},
     {group, d_json}].

%%--------------------------------------------------------------------
%% TEST CASES
%%--------------------------------------------------------------------

t_get_string(_) ->
    ?CHECKSPEC(d_util, get_string, 1).

t_get_binary(_) ->
    ?CHECKSPEC(d_util, get_binary, 1).

t_get_integer(_) ->
    ?CHECKSPEC(d_util, get_integer, 1).

t_get_boolean(_) ->
    ?CHECKSPEC(d_util, get_boolean, 1).

t_get_base62(_) ->
    ?CHECKSPEC(d_util, get_base62, 1).

t_get_base36(_) ->
    ?CHECKSPEC(d_util, get_base36, 1).

t_required(_) ->
    ?CHECKSPEC(d_util, required, 2).

t_validate_list_of_binaries(_) ->
    ?CHECKSPEC(d_util, validate_list_of_binaries, 1).

t_get_value_3(_) ->
    ?CHECKSPEC(d_util, get_value, 3).

t_get_value_4(_) ->
    ?CHECKSPEC(d_util, get_value, 4).

t_create_uuid(_) ->
    ?CHECKSPEC(d_util, create_uuid, 0).
t_validate_email_address(_) ->
    ?CHECKSPEC(d_util, validate_email_address, 1).

t_validate_email_address_correct(_) ->
    ?PROPTEST(prop_email_address).

prop_email_address() ->
    ?FORALL(E, email(), E =:= d_util:validate_email_address(E)).

t_validate_boolean(_) ->
    ?CHECKSPEC(d_json, validate_boolean, 1).

t_validate_boolean_list(_) ->
    ?CHECKSPEC(d_json, validate_boolean_list, 1).

t_validate_boolean_list_generated(_) ->
    ?PROPTEST(prop_validate_boolean_list_generated).

prop_validate_boolean_list_generated() ->
    ?FORALL(L, d_json_boolean_list(),
            case d_json:validate_boolean_list(L) of
                {error, _} -> false;
                _ -> true
            end).

t_validate_float(_) ->
    ?CHECKSPEC(d_json, validate_float, 1).

t_validate_float_list(_) ->
    ?CHECKSPEC(d_json, validate_float_list, 1).

t_validate_float_list_generated(_) ->
    ?PROPTEST(prop_validate_float_list_generated).

prop_validate_float_list_generated() ->
    ?FORALL(L, d_json_float_list(),
            case d_json:validate_float_list(L) of
                {error, _} ->
                    false;
                _ ->
                    true
            end).

t_validate_integer(_) ->
    ?CHECKSPEC(d_json, validate_integer, 1).

t_validate_integer_list(_) ->
    ?CHECKSPEC(d_json, validate_integer_list, 1).

t_validate_integer_list_generated(_) ->
    ?PROPTEST(prop_validate_integer_list_generated).

prop_validate_integer_list_generated() ->
    ?FORALL(L, d_json_integer_list(),
            case d_json:validate_integer_list(L) of
                {error, _} ->
                    false;
                _ ->
                    true
            end).

t_validate_non_neg_integer(_) ->
    ?CHECKSPEC(d_json, validate_non_neg_integer, 1).

t_validate_non_neg_integer_list(_) ->
    ?CHECKSPEC(d_json, validate_non_neg_integer_list, 1).

t_validate_non_neg_integer_list_generated(_) ->
    ?PROPTEST(prop_validate_non_neg_integer_list_generated).

prop_validate_non_neg_integer_list_generated() ->
    ?FORALL(L, d_json_non_neg_integer_list(),
            case d_json:validate_non_neg_integer_list(L) of
                {error, _} ->
                    false;
                _ ->
                    true
            end).

t_validate_integer_range(_) ->
    ?CHECKSPEC(d_json, validate_integer_range, 2).

t_validate_integer_range_generated(_) ->
    ?PROPTEST(prop_validate_integer_range_generated).

prop_validate_integer_range_generated() ->
    ?FORALL({A, B, I}, {integer(), integer(), d_json_integer()},
            begin
            {Lower, Higher} = if A < B ->
                    {A, B};
                true -> {B, A}
            end, 
            case d_json:validate_integer_range({Lower, Higher}, I) of
                {error, _} ->
                    if I < Lower orelse I > Higher -> true;
                        true -> false
                    end;
                _ ->
                    true
            end
        end).


t_validate_binary(_) ->
    ?CHECKSPEC(d_json, validate_binary, 1).

t_validate_binary_list(_) ->
    ?CHECKSPEC(d_json, validate_binary_list, 1).

t_validate_binary_list_generated(_) ->
    ?PROPTEST(prop_validate_binary_list_generated).

prop_validate_binary_list_generated() ->
    ?FORALL(L, list(binary()),
            case d_json:validate_binary_list(L) of
                {error, _} ->
                    false;
                _ ->
                    true
            end).

t_validate_undefined(_) ->
    ?CHECKSPEC(d_json, validate_undefined, 1).

t_validate_undefined_generated(_) ->
    ?PROPTEST(prop_validate_undefined_generated).

prop_validate_undefined_generated() ->
    ?FORALL(U, any(), U =:= d_json:validate_undefined(U)).

t_validate_null(_) ->
    ?CHECKSPEC(d_json, validate_null, 1).

t_validate_null_generated(_) ->
    ?PROPTEST(prop_validate_null_generated).

prop_validate_null_generated() ->
    ?FORALL(U, weighted_union([{1,null}, {10, any()}]), ?IMPLIES(U =:= null, d_json:validate_null(U) =:= undefined)).

t_validate_ignore(_) ->
    ?CHECKSPEC(d_json, validate_ignore, 1).

%% generator for expected json boolean values
binary_boolean() ->
    ?LET(B, oneof([true_list(), false_list()]), list_to_binary(B)).
true_list() ->
    [oneof([$t, $T]),oneof([$r, $R]),oneof([$u, $U]),oneof([$e, $E])].
false_list() ->
    [oneof([$f, $F]),oneof([$a, $A]),oneof([$l, $L]),oneof([$s, $S]),oneof([$e, $E])].
d_json_boolean() ->
    oneof([boolean(), binary_boolean()]).
d_json_boolean_list() ->
    list(d_json_boolean()).

t_get_value_4_generated(_) ->
    ?PROPTEST(prop_get_value_4_generated).

prop_get_value_4_generated() ->
    ?FORALL(T, tuple_list(),
            case d_util:get_value(foo, 1, undefined, T) of
                undefined ->
                    true;
                bar ->
                    false;
                _ ->
                    false
            end).
    
%% generator for expected list of tuples
tuple_list() ->
    ?LET(T, oneof([contains_val_list(), not_contains_val_list()]), T).
contains_val_list() ->
    list({term(), term()}), {foo, bar}, list({term(), term()}).
not_contains_val_list() ->
    list({term(), term()}).

%% generator for expected float
binary_float() ->
    ?LET(B, float(), list_to_binary(float_to_list(B))).
d_json_float() ->
    oneof([float(), binary_float()]).
d_json_float_list() ->
    list(d_json_float()).

%% generator for expected integer
binary_integer() ->
    ?LET(B, integer(), list_to_binary(integer_to_list(B))).
d_json_integer() ->
    oneof([integer(), binary_integer()]).
d_json_integer_list() ->
    list(d_json_integer()).

%% generator for expected non_neg_integer
binary_non_neg_integer() ->
    ?LET(B, non_neg_integer(), list_to_binary(integer_to_list(B))).
d_json_non_neg_integer() ->
    oneof([non_neg_integer(), binary_non_neg_integer()]).
d_json_non_neg_integer_list() ->
    list(d_json_non_neg_integer()).

t_validate_atom(_) ->
    ?CHECKSPEC(d_json, validate_atom, 2).

t_validate_atom_from_list(_) ->
    ?CHECKSPEC(d_json, validate_atom_from_list, 2).

t_validate_url(_) ->
    Url = <<"http://foo.bar.com?a=b">>,
    Url = d_util:validate_url(Url),
    Url2 = <<"https://foo.bar.com?a=b">>,
    Url2 = d_util:validate_url(Url2),
    BadUrl = <<"htt://foo.bar.com?a=b">>,
    {error, {?INVALID_URL, [BadUrl]}} = d_util:validate_url(BadUrl).

t_validate_tcp_port(_) ->
    ?CHECKSPEC(d_util, validate_tcp_port, 1).

t_validate_utf8(_) ->
    ?PROPTEST(prop_validate_utf8).

prop_validate_utf8() ->
    ?FORALL(B, proper_stdgen:utf8_bin(), B =:= d_util:validate_utf8(B)).

t_validate_utf8_spec(_) ->
    ?CHECKSPEC(d_util, validate_utf8, 1).

t_validate_list_with(_) ->
    ?PROPTEST(prop_validate_list_with).

prop_validate_list_with() ->
    ?FORALL(L, list(integer()), L =:= d_util:validate_list_with({d_json, validate_integer}, L)).

t_validate_area_code(_) ->
    ?PROPTEST(prop_validate_area_code).

prop_validate_area_code() ->
    ?FORALL(L, [integer($2, $9), integer($0, $9), integer($0, $9)],
            begin
                AreaCode = list_to_binary(L),
                AreaCode =:= d_util:validate_area_code(AreaCode)
            end).

email_local_part() ->
    non_empty(list(oneof([integer($a, $z), integer($A, $Z), integer($0, $9), $-, $!, $#, $$, $%, $&, $', $*, $/, $=, $?, $^, $_, $`, ${, $|, $}, $~, $-]))).

label() ->
    non_empty(list(oneof([integer($a, $z), integer($A, $Z), integer($0, $9), $-]))).

email_domain() ->
  ?SUCHTHAT(Hostname,
    ?LET(Labels,
      non_empty(list(label())),
      string:join(Labels, ".")),
    length(Hostname) < 256).

email() ->
    ?SUCHTHAT(Email,
              ?LET({LocalPart, Domain}, {email_local_part(), email_domain()},
                   list_to_binary(string:join([LocalPart, Domain], "@"))),
              byte_size(Email) < 255).
